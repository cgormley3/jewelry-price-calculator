...
export default function Home() {
  ...
  const syncToMarket = async (id: string, metals: any[], labor: number, strategyUsed: string, mult: number) => {
    const fresh = calculateFullBreakdown(metals, 0, 0, 0);
    const newRetail = (fresh.totalMaterials + labor) * (strategyUsed === 'A' ? mult : 2);
    const { error } = await supabase.from('inventory').update({
        wholesale: fresh.wholesaleA + labor,
        retail: newRetail
    }).eq('id', id);
  ...
  const addToInventory = async () => {
    ...
    const newItem = {
      ...
      materials_at_making: b.totalMaterials,
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, { 
      startY: 30, 
      head: [['Item', 'Mat. (Orig)', 'Labor', 'Retail (Orig)', 'Retail (Live)', 'Date']], 
      body: tableData,
    ...
    <div className="min-h-screen bg-stone-50 p-4 md:p-10 text-slate-900 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white px-4 py-6 md:px-6 rounded-[2rem] border-2 shadow-sm gap-4 mb-6 relative border-[#A5BEAC]">
          <div className="flex flex-col items-center w-full md:w-auto">
            <div className="flex flex-col items-center leading-none">
              <h1 className="text-2xl font-black uppercase italic tracking-[0.1em] text-slate-900 leading-none ml-[0.1em]">THE VAULT</h1>
              <a href="https://bearsilverandstone.com" target="_blank" rel="noopener noreferrer" className="text-[8px] font-black uppercase tracking-[0.12em] text-stone-400 mt-1 whitespace-nowrap hover:text-[#A5BEAC] transition-colors cursor-pointer">BY BEAR SILVER AND STONE</a>
            </div>
          </div>
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="flex items-center justify-center gap-2 w-full md:w-auto order-1">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400">{!user ? 'Vault Locked' : (user.is_anonymous ? 'Guest' : `User: ${user.email?.split('@')[0]}`)}</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(prices).filter(([name]) => !['success', 'updated_at', 'lastUpdated', 'id'].includes(name)).map(([name, p]) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-xl space-y-5 border-2 border-[#A5BEAC]">
              ...
              <div className="p-4 bg-stone-50 rounded-2xl border-2 border-dotted border-stone-300 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {useManualPrice && (
                    <input type="number" placeholder={`Custom Price per ${tempUnit}`} className="w-full p-3 border border-[#A5BEAC] rounded-xl text-sm outline-none animate-in fade-in" value={manualPriceInput} onChange={(e) => setManualPriceInput(e.target.value === '' ? '' : Number(e.target.value))} />
                  )}
                </div>
              ...
              <div className="mt-4 flex flex-col items-center gap-4">
                <div className="w-full p-4 rounded-xl bg-stone-100 border border-stone-200 space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-stone-200"><span className="text-stone-500 font-bold uppercase text-[10px]">Materials Total</span><span className="font-black text-slate-900">${b.totalMaterials.toFixed(2)}</span></div>
                  <div className="flex justify-between items-center py-2"><span className="text-stone-500 font-bold uppercase text-[10px]">Labor Total ({hours || 0}h)</span><span className="font-black text-slate-900">${b.labor.toFixed(2)}</span></div>
                </div>
                <div className="grid grid-cols-1 gap-4 mb-6 w-full">
                  <button onClick={() => setStrategy('A')} className={`group flex flex-col sm:flex-row sm:items-center sm:justify-between p-5 rounded-[2rem] border-2 transition-all ${strategy === 'A' ? 'border-[#2d4a22] bg-stone-50 shadow-md' : 'border-stone-100 bg-white hover:border-stone-200'}`}><div className="text-left mb-4 sm:mb-0"><p className="text-[10px] font-black opacity-40 uppercase tracking-tighter mb-1 text-slate-900">Retail A</p><p className="text-3xl font-black text-slate-900">${a?.retailA ? a.retailA.toFixed(2) : '0.00'}</p></div><div className="text-left sm:text-right space-y-2"><p className="text-[9px] font-bold text-stone-400 uppercase">Wholesale: M + L</p><div className="flex items-center sm:justify-end gap-2"><span className="text-[10px] font-black text-[#2d4a22] uppercase italic whitespace-nowrap">Retail: W ×</span><input type="number" className="w-12 bg-white border-2 border-[#2d4a22] rounded-xl text-xs font-black text-[#2d4a22] py-1.5 text-center outline-none" value={retailMultA} onChange={(e) => setRetailMultA(Number(e.target.value))} onClick={(e) => e.stopPropagation()} /></div></div></button>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="flex justify-between items-center bg-white px-6 py-4 rounded-2xl border-2 shadow-sm border-[#A5BEAC]"><h2 className="text-lg font-black uppercase tracking-tight text-slate-900">Saved Pieces</h2><button onClick={exportToPDF} disabled={inventory.length === 0} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase hover:bg-[#A5BEAC] transition-colors">Export PDF</button></div>
                        ...
                        <div className="p-6 flex justify-between items-center gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full border ${isUp ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                                    {isUp ? '▲' : '▼'} ${Math.abs(priceDiff).toFixed(2)}
                                </span>
                            </div>
                            <p className="text-[10px] text-stone-400 font-bold uppercase tracking-widest">{new Date(item.created_at).toLocaleDateString()} | Strategy: {item.strategy}</p>
                          </div>
                          <div className="flex gap-4 md:gap-8 text-right items-center">
                            <div className="hidden md:block text-slate-400">
                                <p className="text-[9px] font-black uppercase">Orig. Retail</p>
                                <p className="text-sm font-bold line-through">${Number(item.retail).toFixed(2)}</p>
                            </div>
                            <div>
                                <p className="text-[9px] font-black text-[#2d4a22] uppercase italic">Live Value</p>
                                <p className="text-2xl font-black text-slate-900">${liveRetail.toFixed(2)}</p>
                            </div>
                            <button onClick={() => deleteInventoryItem(item.id)} className="text-red-300 hover:text-red-600 transition-colors text-lg">×</button>
                          </div>
                        </div>
                        <details className="group border-t border-stone-50">
                            <summary className="list-none cursor-pointer py-2 text-center text-[8px] font-black uppercase tracking-[0.2em] text-stone-300 hover:text-[#A5BEAC] transition-colors">Breakdown & History</summary>
                            <div className="p-6 bg-stone-50/50 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    <h4 className="text-[10px] font-black uppercase text-stone-400">Components</h4>
                                    {item.metals?.map((m: any, idx: number) => (
                                        <div key={idx} className="flex justify-between text-[10px] font-bold border-b border-stone-100 pb-1 uppercase">
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div className="bg-white p-3 rounded-xl border border-stone-100">
                                            <p className="text-[8px] font-black text-stone-400 uppercase">Orig. Materials</p>
                                            <p className="text-xs font-black text-slate-700">${Number(item.materials_at_making || 0).toFixed(2)}</p>
                                        </div>
                                        <div className="bg-white p-3 rounded-xl border border-stone-100">
                                            <p className="text-[8px] font-black text-stone-400 uppercase">Labor Cost</p>
                                            <p className="text-xs font-black text-slate-700">${Number(item.labor_at_making || 0).toFixed(2)}</p>
                                        </div>
                                    </div>
                                    <button onClick={() => syncToMarket(item.id, item.metals, item.labor_at_making || 0, item.strategy, item.multiplier)} className="w-full py-2 bg-[#2d4a22] text-white rounded-lg font-black text-[9px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-sm">Sync Vault to Market</button>
                                </div>
                            </div>
                        </details>
                      </div>
                ...
                <div className="space-y-2 text-xs text-stone-700">
                  ...
                  <div className="mt-4 p-3 bg-white/50 rounded-xl border border-[#A5BEAC]/30">
                    <p className="text-[10px] text-stone-500 leading-relaxed">
                      ...
                      Standard industry retail markup is typically 2.0 to 3.0 times the wholesale cost to cover overhead and business growth.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-6 rounded-2xl border-2 transition-all ${strategy === 'B' ? 'border-[#A5BEAC] bg-stone-50' : 'bg-stone-50 border-stone-100'}`}>
                ...
                <div className="space-y-2 text-xs text-stone-700">
                  ...
                  <div className="mt-4 p-3 bg-white/50 rounded-xl border border-[#A5BEAC]/30">
                    <p className="text-[10px] text-stone-500 leading-relaxed">
                      <span className="font-black text-[#A5BEAC] uppercase text-[8px] block mb-1">Industry Standard:</span>
                      A production markup of 1.5 to 1.8 is standard to account for metal loss, consumables, and market volatility before labor is applied.
                    </p>
                  </div>
                </div>
              </div>
